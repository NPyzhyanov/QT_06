...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        */
        ...
        fut = QtConcurrent::run(MainWindow::startCalculations, concurRace1, &number, ui->sb_initNum->value())
                .then(MainWindow::startCalculations, concurRace1, &number, ui->sb_initNum->value());
...
void MainWindow::startCalculations(ExampleRace* race_obj, uint32_t* num, int iterations_amount)
{
    race_obj->DoWork(num, false, iterations_amount);
}
